...
class BernoulliEnv(BaseEnv):
	def __init__(self, X, tilde_X, arm_affinity_matrix, arm_to_suparms, suparm_to_arms, arms, suparms,
		...
		super().__init__(X, tilde_X, arm_affinity_matrix, arm_to_suparms, suparm_to_arms, arms, suparms,
	...
	def _get_relative_reward(self, user_idx, picked_arm_idx, duel_arm_idx, is_suparm):
		...
		if is_suparm:
			picked_affinity = self.suparm_affinity_matrix[user_idx,
														  picked_arm_idx]
			duel_affinity = self.suparm_affinity_matrix[user_idx, duel_arm_idx]
		else:
			picked_affinity = self.arm_affinity_matrix[user_idx,
													   picked_arm_idx]
	...
	def _get_hucb_supreward(self, user_idx, suparm_idx):
		picked_affinity = self.suparm_affinity_matrix[user_idx, suparm_idx]
		best=np.max(self.suparm_affinity_matrix[user_idx])
		print('best',best,'pick',picked_affinity)
		return best,picked_affinity