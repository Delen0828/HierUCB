...
class Con_UCB:
    ...
    def decide_suparms(self, pool_suparm, uid, norm, arms,X_t,debug_fw=None):
        ...
        if self.suparm_strategy=='random':
            ...
            for x, xinfo in pool_suparm.items():
                x_pta=self.users[uid].getCredit(xinfo.fv[uid])
                if x_pta>max_C:
                    picked_suparm=xinfo
                    max_C=x_pta
            ...
            for x, xinfo in pool_suparm.items():
                new_tildeMinv=self.users[uid].getInv(cur_tildeMinv,xinfo.fv[uid])
    ...
    def updateSuparmParameters(self,picked_arm,reward, uid):
        self.users[uid].updateSuparmParameters(picked_arm.fv[uid], reward)
